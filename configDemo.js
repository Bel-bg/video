...
async function configDemo() {
  try {
    ...
    console.log(
      `  Performance Monitoring: ${config.get(
        "features.enablePerformanceMonitoring"
      )}`
    );
    console.log(
      `  Max Videos Per Page: ${config.get("limits.maxVideosPerPage")}`
    );
    console.log(
      `  Max Comments Per Page: ${config.get("limits.maxCommentsPerPage")}`
    );
