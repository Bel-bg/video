...
class MetricsCollector {
  ...
  getStats() {
    ...
    const errorRate = totalCalls > 0 ? (this.metrics.apiErrors / totalCalls * 100).toFixed(2) : 0;
    const avgResponseTime = totalCalls > 0 ? (this.metrics.totalResponseTime / totalCalls).toFixed(2) : 0;
...
class MonitoredApi {
  ...
  async call(operationName, apiCall, ...args) {
    ...
    try {
      ...
      console.log(`  ${operationName} completed in ${duration}ms (${success ? 'SUCCESS' : 'FAILED'})`);
    }
  }
...
async function monitoringDemo() {
  try {
    ...
    metrics.recentCalls.forEach(call => {
      console.log(`    ${call.operation}: ${call.duration}ms (${call.success ? 'SUCCESS' : 'FAILED'}) at ${call.timestamp}`);
    });
