...
async function backupRestoreDemo() {
  try {
    ...
    const videoDetails = [];
    for (const video of feed.slice(0, 3)) {
      ...
      try {
        ...
        console.log(
          `  Failed to get details for ${video.id}: ${error.message}`
        );
      }
    }
    ...
    console.log(
      "  🔄 In a real app, this would recreate the videos and their data"
    );
    ...
    console.log(
      `  File size: ${(JSON.stringify(backupData).length / 1024).toFixed(2)} KB`
    );
