...
async function workflowTest() {
  try {
    ...
    console.log(
      `  ✅ Video uploaded successfully with ID: ${uploadedVideo.id}`
    );
    ...
    console.log(
      `  ✅ Video ${likeResult.liked ? "liked" : "unliked"} successfully`
    );
    ...
    console.log(
      `  ✅ Video ${unlikeResult.liked ? "liked" : "unliked"} successfully`
    );
    ...
    for (let i = 0; i < comments.length; i++) {
      const comment = await videoApi.commentOnVideo(
        uploadedVideo.id,
        comments[i]
      );
    ...
    const commentsData = await videoApi.getVideoComments(
      uploadedVideo.id,
      1,
      10
    );
    ...
    commentsData.comments.forEach((comment, index) => {
      console.log(
        `    ${index + 1}. ${comment.text} (by ${comment.user.username})`
      );
    });
    ...
    console.log(
      `  Comments after adding: ${videoAfterComments.comments_count}`
    );
    ...
    const ourVideoInFeed = updatedFeed.find(
      (video) => video.id === uploadedVideo.id
    );
    if (ourVideoInFeed) {
      console.log(
        `  Our video is in the feed with ${ourVideoInFeed.likes_count} likes and ${ourVideoInFeed.comments_count} comments`
      );
    } else {
    ...
    try {
      ...
      if (
        ...
        console.log(
          "  ❓ Unexpected error when checking deleted video:",
          error.message
        );
      }
    }
