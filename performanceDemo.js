...
async function performanceDemo() {
  try {
    ...
    const feed = await measurePerformance("Get video feed", () =>
      videoApi.getVideoFeed(5)
    )();
    ...
    if (feed.length > 0) {
      ...
      await measurePerformance("Get video details", () =>
        videoApi.getVideo(videoId)
      )();

      await measurePerformance("Like video", () =>
        videoApi.likeVideo(videoId)
      )();

      await measurePerformance("Add comment", () =>
        videoApi.commentOnVideo(videoId, "Performance test comment")
      )();

      await measurePerformance("Get comments", () =>
        videoApi.getVideoComments(videoId, 1, 5)
      )();

      await measurePerformance("Share video", () =>
        videoApi.shareVideo(videoId, "performance_test")
      )();
      ...
      const concurrentOperations = [
        measurePerformance("Concurrent op 1", () =>
          videoApi.getVideo(videoId)
        )(),
        measurePerformance("Concurrent op 2", () =>
          videoApi.likeVideo(videoId)
        )(),
        measurePerformance("Concurrent op 3", () =>
          videoApi.commentOnVideo(videoId, "Concurrent test comment")
        )(),
      ];
      ...
      console.log(
        `  All concurrent operations: ${concurrentEnd - concurrentStart}ms`
      );
    }
