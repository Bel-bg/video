  ...
  try {
    ...
    if (feed.length > 0) {
      ...
      const operations = feed.slice(0, 3).map(async (video, index) => {
        console.log(`  Starting operations on video ${index + 1} (${video.id})`);
        ...
        // Add a comment
        const commentResult = await videoApi.commentOnVideo(video.id, `Concurrent comment ${index + 1}`);
        
        // Share the video
        const shareResult = await videoApi.shareVideo(video.id, `platform_${index + 1}`);
