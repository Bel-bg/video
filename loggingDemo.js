...
function loggedApiCall(name, apiCall) {
  return async (...args) => {
    ...
    try {
      ...
      const end = Date.now();
      logger.info(`Completed ${name}`, { duration: `${end - start}ms`, result: typeof result === 'object' ? `${JSON.stringify(result).substring(0, 100)}...` : result });
      return result;
    } catch (error) {
      const end = Date.now();
      logger.error(`Failed ${name}`, { duration: `${end - start}ms`, error: error.message });
      throw error;
    }
  };
}
...
async function loggingDemo() {
  try {
    ...
    const loggedGetVideoFeed = loggedApiCall('getVideoFeed', videoApi.getVideoFeed);
    ...
    const loggedCommentOnVideo = loggedApiCall('commentOnVideo', videoApi.commentOnVideo);
    ...
    if (feed.length > 0) {
      ...
      const commentResult = await loggedCommentOnVideo(videoId, 'Logging demo comment');
